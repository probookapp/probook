        ...
        <View style={styles.table}>
          ...
          {(() => {
            ...
            Object.entries(groupedLines).forEach(([groupName, lines]) => {
              ...
              lines.forEach(line => {
                ...
                elements.push(
                  ...
                  >
                    ...
                    <Text style={styles.colUnitPrice}>{formatCurrency(line.unit_price_ht)}</Text>
                    <Text style={styles.colVat}>{line.vat_rate}%</Text>
                    <Text style={styles.colTotal}>{formatCurrency(line.total_ttc)}</Text>
                  </View>
                );
                rowIndex++;
              });
              ...
              // Subtotal for group
              const groupTotalHt = lines.reduce((sum, l) => sum + l.total_ht, 0);
              const groupTotalTtc = lines.reduce((sum, l) => sum + l.total_ttc, 0);
              elements.push(
          ...
          <View style={styles.totalsBox}>
            ...
            {quote.shipping_cost_ht > 0 && (
              <>
                <View style={styles.totalRow}>
                  ...
                  <Text style={styles.totalValue}>
                    {formatCurrency(quote.shipping_cost_ht)}
                  </Text>
                </View>
                <View style={styles.totalRow}>
                  <Text style={styles.totalLabel}>{t("quote.totals.shippingVat")} ({quote.shipping_vat_rate}%)</Text>
                  <Text style={styles.totalValue}>
                    {formatCurrency(quote.shipping_cost_ht * (quote.shipping_vat_rate / 100))}
                  </Text>
                </View>
              </>
            )}
            <View style={styles.totalRowFinal}>
              ...
              <Text style={styles.totalValueFinal}>
                {formatCurrency(
                  quote.total_ttc +
                  (quote.shipping_cost_ht > 0
                    ? quote.shipping_cost_ht + quote.shipping_cost_ht * (quote.shipping_vat_rate / 100)
                    : 0)
                )}
              </Text>
            </View>
            {(quote.down_payment_percent > 0 || quote.down_payment_amount > 0) && (() => {
              const totalTtcWithShipping = quote.total_ttc +
                (quote.shipping_cost_ht > 0
                  ? quote.shipping_cost_ht + quote.shipping_cost_ht * (quote.shipping_vat_rate / 100)
                  : 0);
              const downPaymentValue = quote.down_payment_amount > 0
                ? quote.down_payment_amount
                : totalTtcWithShipping * (quote.down_payment_percent / 100);
              return (
                <>
                  ...
                  <View style={styles.totalRow}>
                    ...
                    <Text style={styles.totalValue}>
                      {formatCurrency(totalTtcWithShipping - downPaymentValue)}
                    </Text>
                  </View>
                </>
              );
            ...
            <View style={{ marginTop: 8, paddingTop: 8, borderTopWidth: 1, borderTopColor: "#e5e7eb" }}>
              <Text style={{ fontSize: 8, color: "#6b7280", fontStyle: "italic" }}>
                {t("quote.amountInWords")}: {numberToFrenchWords(
                  quote.total_ttc +
                  (quote.shipping_cost_ht > 0
                    ? quote.shipping_cost_ht + quote.shipping_cost_ht * (quote.shipping_vat_rate / 100)
                    : 0)
