        ...
        <View style={styles.totalsContainer}>
          <View style={styles.totalsBox}>
            ...
            {invoice.shipping_cost_ht > 0 && (
              <>
                <View style={styles.totalRow}>
                  ...
                  <Text style={styles.totalValue}>
                    {formatCurrency(invoice.shipping_cost_ht)}
                  </Text>
                </View>
                <View style={styles.totalRow}>
                  <Text style={styles.totalLabel}>{t("invoice.totals.shippingVat")} ({invoice.shipping_vat_rate}%)</Text>
                  <Text style={styles.totalValue}>
                    {formatCurrency(invoice.shipping_cost_ht * (invoice.shipping_vat_rate / 100))}
                  </Text>
                </View>
              </>
            )}
            ...
            {(invoice.down_payment_percent > 0 || invoice.down_payment_amount > 0) && (() => {
              const totalTtcWithShipping = invoice.total_ttc +
                (invoice.shipping_cost_ht > 0
                  ? invoice.shipping_cost_ht + invoice.shipping_cost_ht * (invoice.shipping_vat_rate / 100)
                  : 0);
              const downPaymentValue = invoice.down_payment_amount > 0
                ? invoice.down_payment_amount
                : totalTtcWithShipping * (invoice.down_payment_percent / 100);
              return (
                <>
